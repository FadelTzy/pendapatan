...
class RkaklController extends Controller
{
    public function check($id)
    {
        ...
        $tes = rkakl::where('aktivasi',1)->first();
        if ($tes) {
            # code...
            $tes->aktivasi = 0;
            $tes->save();
    ...
    public function indexv2()
    {
        ...
        if (request()->ajax()) {
            return Datatables::of(rkakl::with('revici')->get())->addIndexColumn()->addColumn('aksi', function ($data) {
                ...
                // $btn .= "  <button type='button' data-toggle='modal' onclick='staffcheck(" . $data->id . ")'   class='btn d-inline btn-sm btn-primary btn-xs mb-1'> BKU </button>";
                // $btn .= "  <button type='button' data-toggle='modal' onclick='staffcheck(" . $data->id . ")'   class='btn d-inline btn-sm btn-primary btn-xs mb-1'> BP </button>";
                // $btn .= "  <button type='button' data-toggle='modal' onclick='staffcheck(" . $data->id . ")'   class='btn d-inline btn-sm btn-primary btn-xs mb-1'> Referensi </button>";
            ...
            if ($data) {
                if ($data->aktivasi == 1) {
                    ...
                    $btn =  '<button class="btn-sm btn-danger btn "> Non </button>';
                }
            }else{
                $btn = '-';
            }
            ...
            addColumn('tanggalnya', function ($data) {
                ...
                if ($data) {
                    $date = Carbon::parse($data->tgl)->locale('id');
