...
class SppController extends Controller
{
    ...
    {
        $id = $request->id;
        $rs = RiwayatSpp::where('id', $request->id)->first();
        $crm = RiwayatMaster::where('id_riwayat_spp', $id)->first();
        $cps = pajakSpp::where('id_riwayat_spp', $id)->delete();
        if ($rs->status == 0) {
            # code...
        }
        if ($crm) {
        ...
        if ($rs->jenis == 2) {
            up::where('idriwayat', $id)->delete();
        }
        if ($rs->jenis == 1) {
            $rsppd = rsppd::where('id_riwayat_spp', $id)->get();
            foreach ($rsppd as $rsppdv) {
                $id_rsppd = $rsppdv->id_rab_detail;
                if ($rsppdv->status == 1) {
                    $rabd = rab_detail::where('id', $id_rsppd)->first();
                    $rabd->realisasi = $rabd->realisasi - $rsppdv->sppini;
                    $rabd->sisabiaya = $rabd->sisabiaya + $rsppdv->sppini;
                    $rabd->save();
                }
                $rsppdv->delete();
            }
            $rspp = rspp::where('id_riwayat_spp', $id)->first();
            // return $rspp;
            if ($rspp) {
                $rspp->delete();
                # code...
    ...
    {
        if (request()->ajax()) {
            return Datatables::of(Spp::with('akunrk', 'dataakunrkakl.rab.kegiatanr')->get())
                ->addIndexColumn()
                ...
                <a type='button'  href='" .
                        ...
                        '/' .
                        $data->id .
                        "'   class='btn btn-primary btn-sm btn-xs mb-1'>Cetak </a>
                </li>
                <li class='list-inline-item'>
                ...
                ->rawColumns(['aksi', 'nama_akunrk', 'wow', 'no_tglspk'])
                ->make(true);
        }
    ...
    {
        $data['arr'] = Spp::with('sp', 'dp', 'cb', 'js', 'jp', 'pejabat_pk', 'pejabat_pp', 'supplier', 'kp', 'dataakunrkakl.rab.kegiatanr', 'dataakunrkakl.rab.kror', 'dataakunrkakl.rab.ror', 'dataakunrkakl.rab.komponenr', 'dataakunrkakl.rab.akunr')
            ->where('id', $id)
            ->first();
        $data['profil'] = Setting::first();
        $data['rkakl'] = rkakl::with('datarab', 'datarab.datakro.dataakunrkakl', 'datarab.kegiatanr', 'datarab.kror')
            ->where('aktivasi', 1)
            ->first();
        // return $data['rkakl'];
        // return $data;
    ...
    {
        $data['arr'] = Spp::with('sp', 'jb', 'jp', 'pejabat_pk', 'pejabat_pp', 'supplier', 'kp', 'dataakunrkakl.rab.kegiatanr', 'dataakunrkakl.rab.kror', 'dataakunrkakl.rab.ror', 'dataakunrkakl.rab.komponenr', 'dataakunrkakl.rab.akunr')
            ->where('id', $id)
            ->first();
        $data['profil'] = Setting::first();
        $data['rkakl'] = rkakl::with('datarab', 'datarab.datakro.dataakunrkakl', 'datarab.kegiatanr', 'datarab.kror')
    ...
    {
        $datark = rkakl::with('datarab', 'datarab.kegiatanr', 'datarab.kror')
            ->where('aktivasi', 1)
            ->first();
