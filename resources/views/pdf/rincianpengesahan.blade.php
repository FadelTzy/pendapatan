...
@endsection
@section('content')
<table class="table table-hover table-sm table-bordered align-middle table-row-dashed fs-6 gy-5" id="customers">
    <!--begin::Table head-->
    <thead>
        <!--begin::Table row-->
    ...
    <tbody>
        ...
        @foreach ($data as $item)
            <tr>
                <td>{{ $nom++ }}</td>
                <td>  {{ \Carbon\Carbon::parse($item->tanggal_p)->isoFormat('DD MMM YYYY') }}</td>
                <td>{{ $item->nomor }}</td>
                @php
                    $jum = 0;
